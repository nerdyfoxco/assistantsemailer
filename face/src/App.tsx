...
import { Loader2, CheckCircle, Brain, RefreshCcw, Key } from 'lucide-react';
import { KeyManager } from './chapters/12_developer/KeyManager';
...
function App() {
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-gray-100 font-sans">
      {/* Sidebar: Global Nav */}
      <div className="w-16 flex flex-col items-center py-4 border-r border-gray-800 bg-gray-900 z-10">
        <Link to="/" className="p-3 bg-blue-600 rounded-xl mb-4 text-white hover:bg-blue-500 transition-colors">
          <RefreshCcw className="w-6 h-6" />
        </Link>
        <Link to="/developer" className="p-3 rounded-xl text-gray-400 hover:text-white hover:bg-gray-800 transition-colors">
          <Key className="w-6 h-6" />
        </Link>
      </div>

      <Routes>
        <Route path="/developer" element={<div className="flex-1 p-8 bg-background text-foreground"><KeyManager /></div>} />
        <Route path="/" element={
